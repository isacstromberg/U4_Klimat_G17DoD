    ...
    public class Db_Repository
    {
        ...
        public void RegisterObservation(int? Observer_ID_input, double? Geolocation_ID_input)
        {
            ...
            catch (PostgresException ex)
            {
                if (ex.SqlState == "23503")
                {
                    throw new Exception("Angivet Observer_ID stämmer inte överrens med databasen", ex);
                }
                throw new Exception("Allvarligt fel, får inte kontakt med databasen", ex);
            }
        ...
        public void UpdateFjallripaSuit(int? input_value, int? id)
        {
            try
            {
                using var conn = new NpgsqlConnection(_connectionString);
                conn.Open();
                StringBuilder sql = new StringBuilder("UPDATE measurement ");
                sql.AppendLine("SET category_id=@category_id ");
                sql.AppendLine("WHERE id=@id");
                using var command = new NpgsqlCommand(sql.ToString(), conn);
                command.Parameters.AddWithValue("@category_id", input_value);
                command.Parameters.AddWithValue("@id", id);
                var result = command.ExecuteScalar();
            }
            catch (Exception)
        ...
        public bool BlockNumbers(string input)
        {
